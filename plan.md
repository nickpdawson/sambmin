...
**M22: Security Hardening** (2026-02-22)
- [x] Global auth middleware: all API endpoints require authentication (except health, login)
- [x] RBAC: 4 roles (Authenticated, Operator, DNSAdmin, Admin) with `RequireRole()` middleware
- [x] Input validation module: SAMAccountName, GroupName, DNSName, DNSType, SPN, Password, BaseDN, RawFilter
- [x] LDAP filter injection protection: balanced parens, max length, null byte rejection, attribute blocklist
- [x] Login rate limiting: 10 failed/min per-IP, 5 failed/15min per-username (429 + Retry-After)
- [x] Double-submit cookie CSRF protection (sambmin_csrf cookie + X-CSRF-Token header)
- [x] Frontend API client sends CSRF token on all mutation requests
- [x] Timing-safe password comparison (subtle.ConstantTimeCompare)
- [x] Error message sanitization: respondSafeError across all 15 handler files (~68 instances)
- [x] Secure keytab temp files (os.MkdirTemp + defer os.RemoveAll)
- [x] Credential filtering in debug logs (filterSensitiveArgs)
- [x] HSTS + Content-Security-Policy headers in nginx
- [x] Log file permissions 0644 → 0640
- [x] New files: auth/rbac.go, validate/validate.go, middleware/ratelimit.go (+ tests)
